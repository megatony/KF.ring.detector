...
figure(1); imshow(uint8(ImgQ));
hold on;
DrawLine(Region);
hold off;
figure(2);
separatedImage = getKFRing(Region,image_org,10);
comparisionImage = createComparisionImage(separatedImage);
comparisionImage1 = im2bw(separatedImage);
getKFPercent(comparisionImage,comparisionImage1)